...
setwd("/Volumes/som-ts/GynOb/SmithLab/NewData/People/Dasia/RUSH/Step2_CellComposition")

#if (!require("BiocManager", quietly = TRUE))
#  install.packages("BiocManager")

# need these for this script DYS
...
# get beta values from enmix data folder and convert rdata item to csv file #DYS
load("RUSH_QC_DYS_Nov142024_betas_without_suffix_passQC.RData")
#write.csv(BETAFILE_PASS,file = "RUSH_QC_beta_without_suffix_nov20_2024.csv")

## Load beta values (not ComBAT adjusted beta values, just normalized beta values)
beta <- fread("RUSH_QC_beta_without_suffix_nov20_2024.csv", data.table = F)
...
pheno <- read.csv("RUSH_QC_DYS_Nov142024_pheno_ENmix_passQC_withscores.csv")
#pheno <- pheno[,-1]
## Run the function
output <- estimate_cell_proportion(betavals = beta, phenotyps = pheno, cell_prop_ref = ref, xcol = "SampleID", ycol = 'row.names' )
write.csv(output, file = "RUSH_Pheno_QC_withCell_types_Nov20_24.csv", row.names = FALSE) # Save the phenotype file with cell types
